...
- 设置目标期数后可查看预计总投入、剩余投入和预计完成日期；定投概览可对照近 30 天实际投入、近 6 个自然月投入趋势、近 90 天执行稳定性与未来 30 天计划投入。人民币与美元分别汇总且趋势独立缩放；执行稳定性区分计划执行、补执行和手动执行，按时率不包含手动及缺少执行类型的旧流水；未来投入展示预计期数、涉及计划数，并按计划展示预计投入、单期金额及首末执行日期，暂停、归档和已完成计划不计入未来投入。
- 展开单个定投计划可查看该计划近 90 天按时率、补执行和手动执行情况；统计只包含该计划来源流水，没有自动执行时按时率显示 `--`，手动执行和缺少执行类型的旧流水不计入按时率。计划首次执行后还可查看累计投入、手续费、执行次数、累计数量、定投均价、当前市值、盈亏和最近 10 条执行记录；普通手动流水不计入计划绩效。
...
- 定投计划可设置目标期数；已完成和剩余期数展示准确，跳过本期不计入目标，达到目标后自动完成且不能继续手动或自动执行。
- 定投计划列表可按运行中、待执行、需处理、已暂停和已完成筛选，并按状态优先、下次执行、最近更新、累计投入或名称排序；默认状态优先先显示需处理和待执行计划，已归档列表不显示进行中状态筛选。
- 设置目标期数后可查看预计总投入、剩余投入和预计完成日期；定投概览可分别汇总未来 30 天人民币与美元的计划投入、预计期数和涉及计划数，并按计划展示预计投入、单期金额及首末执行日期；暂停、归档和已完成计划不计入。
- 展开单个定投计划可查看该计划近 90 天按时率、补执行和手动执行情况；统计只包含该计划来源流水，没有自动执行时按时率显示 `--`，手动执行和缺少执行类型的旧流水不计入按时率。计划首次执行后还可查看累计投入、手续费、执行次数、累计数量、定投均价、当前市值、盈亏和最近 10 条执行记录；普通手动流水不计入计划绩效。
- 单个定投计划可将全部执行记录导出为 CSV；导出内容只包含该计划生成的定投买入流水，并显示实际保存路径。
- 无有效行情时计划显示等待行情，删除关联持仓时自动暂停；应用关闭期间只补最近一期，重新启用后不补停用期间的历史期次。
- 今日概览可查看到期或异常计划及当日执行结果，并可跳转到对应计划；每日摘要预览包含计划汇总、下一次计划、最近执行和异常计划。
