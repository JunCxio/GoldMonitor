...
- 行情质量持续异常通知默认启用、门槛为 5 分钟且仅选择本机渠道；启停、门槛、本机/邮件/Webhook 渠道和恢复通知设置可保存并在刷新后恢复，启用时不允许所有渠道均为空。
- 同一轮异常达到门槛后只生成一次警报；应用重启后续接已累计的运行时长且停机时间不计入，恢复后自动关闭原异常待处理项，关闭恢复通知时仍会自动关闭但不新增恢复警报。
