...
本文档用于每次版本发布后的验收。执行前确认当前版本号已经在 `goldmonitor/application.py`、`installer/GoldMonitor.iss` 和 `CHANGELOG.md` 中同步，并已推送对应 `vX.Y.Z` 标签。
...
- 局域网只读面板默认关闭；启用时必须设置至少 12 位访问口令，主应用设置页在桌面、中等宽度和窄屏下均能完整展示口令状态、输入框及删除操作，不得裁切或横向溢出。
- 已保存访问口令可标记删除并保存；保存后系统凭据中的口令被清除、局域网面板自动停用，刷新设置页后显示未配置状态，原口令不能继续登录。
- 局域网只读页面只展示白名单行情、质量、规则状态和最近警报摘要，不出现设置、持仓、风险分析、警报处理或其他远程写入入口。
- 从持仓详情的“操作”菜单点击“新增买入”或“新增卖出”后，当前详情立即切换到流水编辑界面，交易类型与关联持仓正确，不需要返回列表。
- 详情流水编辑器取消后保留当前持仓详情；保存后持仓数量、盈亏和关联流水立即刷新；编辑中返回列表会清理未保存草稿。桌面与窄屏均不得出现编辑器隐藏、裁切或横向溢出。
...
- 行情质量事件在应用重启后仍可查看；最近 24 小时和 7 天统计能展示可信区间占比、异常段数、异常时长及历史入库、预警、定投受影响情况。
- 行情质量历史遵守最近 30 天、最多 2000 段保留限制；旧版完整归档不包含该文件时仍可恢复。
- 行情质量持续异常通知默认启用、异常门槛为 5 分钟、恢复确认门槛为 2 分钟且仅选择本机渠道；启停、两个门槛、本机/邮件/Webhook 渠道和恢复通知设置可保存并在刷新后恢复，启用时不允许所有渠道均为空。
- 同一轮异常达到门槛后只生成一次警报；行情连续正常达到恢复确认门槛后才关闭原异常，确认期间再次异常会取消确认且不重复通知。应用重启后续接异常和恢复确认的累计运行时长且停机时间不计入，关闭恢复通知时仍会自动关闭但不新增恢复警报。
- 行情质量异常和恢复警报遵守全局静默时段；警报弹窗、警报记录和今日概览均可直达对应异常区间复盘。
...
- 完整数据归档创建或恢复期间不得与历史数据写入、诊断或修复交叉执行；恢复完成后的内存行情与磁盘历史保持一致，并清理不属于归档内容的旧修复恢复点。
- 历史数据修复可预览并确认执行；预览后改变数据库或 JSON 归档时，即使影响数量不变，旧预览也必须失效且不得写入，重新预览后可正常执行。
- 历史数据修复成功或失败后会写入最近操作记录；执行失败时旧预览关闭并自动刷新诊断，不得继续启用失效的执行按钮。
- 历史数据维护诊断按原始、1 分钟、5 分钟、1 小时和日线展示记录数、采样间隔、保留策略、实际覆盖区间和差异状态；窄屏下不得横向溢出或相互覆盖。
- 成功执行清理、重建或 JSON 同步前会创建最近修复恢复点；恢复预览显示原操作、时间和记录数，确认恢复后 SQLite 回到修复前状态、JSON 不变且恢复点被消费。
- 配置导出和诊断报告导出可生成文件；诊断 JSON 和复制摘要包含历史数据库完整性、明细异常、汇总差异、JSON 可补记录及最近修复恢复点状态。
- 程序内更新检查能识别当前版本。
