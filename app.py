...
def build_alert_rule_simulation(rule_payload, days=30, now=None):
    ...
    if rule.get("kind") == "portfolio":
        ...
        portfolio_history = portfolio_analytics_core.build_portfolio_position_performance(
            transactions,
            history,
...
socket_alert_rules_core.register_alert_rule_handlers(
    ...
    get_alert_rules_state=get_alert_rules_state,
    upsert_alert_rule_entry=upsert_alert_rule_entry,
    delete_alert_rule_entry=delete_alert_rule_entry,
    toggle_alert_rule_entry=toggle_alert_rule_entry,
    duplicate_alert_rule_entry=duplicate_alert_rule_entry,
    reset_alert_rule_entry=reset_alert_rule_entry,
    batch_update_alert_rules_entry=batch_update_alert_rules_entry,
    build_alert_rule_insight=build_alert_rule_insight,
    build_alert_rule_simulation=build_alert_rule_simulation,
    broadcast_alert_rule_views=_broadcast_alert_rule_views,
    find_alert_rule=_find_alert_rule,
)
...
socket_portfolio_core.register_portfolio_handlers(
    ...
    upsert_portfolio_position=lambda data: upsert_portfolio_position(data),
    delete_portfolio_position=lambda position_id: delete_portfolio_position(position_id),
    upsert_portfolio_transaction=lambda data: upsert_portfolio_transaction(data),
    delete_portfolio_transaction=lambda transaction_id: delete_portfolio_transaction(transaction_id),
    preview_import_portfolio_transactions_csv=(
        lambda content: preview_import_portfolio_transactions_csv(content)
    ),
    import_portfolio_transactions_csv=lambda content: import_portfolio_transactions_csv(content),
    undo_portfolio_import=lambda: undo_portfolio_import(),
    portfolio_import_backup_state=lambda data: portfolio_import_backup_state(data),
    get_portfolio_import_backup=lambda: portfolio_import_backup,
    upsert_portfolio_alert=lambda data: upsert_portfolio_alert(data),
    reset_portfolio_alert=lambda alert_id: reset_portfolio_alert(alert_id),
    delete_portfolio_alert=lambda alert_id: delete_portfolio_alert(alert_id),
    broadcast_alert_rule_views=lambda: _broadcast_alert_rule_views(),
    build_portfolio_csv=lambda kind: build_portfolio_csv(kind),
...
socket_alert_log_core.register_alert_log_handlers(
    socketio,
    now_factory=lambda: datetime.now(),
    build_alert_log_csv=lambda: build_alert_log_csv(),
    save_export_file=lambda filename, content: save_export_file(filename, content),
    build_export_error_payload=lambda message: build_export_error_payload(message),
    clear_alert_log_archive=lambda: clear_alert_log_archive(),
    clear_alert_log_memory=lambda: alert_log.clear(),
    update_alert_log_status=(
        lambda alert_id, **kwargs: update_alert_log_status(alert_id, **kwargs)
    ),
    update_alert_log_handling=(
        lambda alert_id, **kwargs: update_alert_log_handling(alert_id, **kwargs)
    ),
    resend_alert_notification=(
        lambda alert_id, **kwargs: resend_alert_notification(alert_id, **kwargs)
    ),
    start_alert_notification_delivery=(
        lambda entry, title: _start_alert_notification_delivery(entry, title)
    ),
    alert_resend_title=lambda entry: _alert_resend_title(entry),
)
...
def _set_floating_hwnd(value):
    global _floating_hwnd
    _floating_hwnd = value
...
def _floating_price_window_loop():
    return floating_runtime_core.run_floating_price_window(
        ...
        window_metrics=lambda: _floating_window_metrics(),
        floating_rect=lambda rect, width, height: _floating_rect(rect, width, height),
        get_text_state=_floating_text_state,
        clamp_position=(
            lambda x, y, user32=None: _clamp_floating_position(x, y, user32)
        ),
        position_window=(
            lambda hwnd, user32=None, x=None, y=None:
            _position_floating_window(hwnd, user32, x, y)
        ),
        save_position=lambda x, y: _save_floating_position(x, y),
        resolve_position=(
            lambda user32, width, height:
            _resolve_floating_position(user32, width, height)
        ),
        set_window_handle=_set_floating_hwnd,
        apply_corner_preference=(
            lambda hwnd: _apply_floating_window_corner_preference(hwnd)
        ),
        apply_opacity=lambda hwnd, user32=None: _apply_floating_opacity(hwnd, user32),
        set_window_visible=lambda visible: _set_floating_window_visible(visible),
        window_enabled=(
            lambda: get_settings_snapshot().get("floating_price_enabled", True)
        ),
        set_ready=lambda: _floating_window_ready.set(),
        show_main_window=lambda: show_main_window(),
        set_enabled=lambda enabled: _set_floating_price_enabled(enabled),
        refresh_price=lambda: _refresh_price_from_floating_menu(),
        open_risk_analysis=lambda: _open_risk_analysis_from_floating_menu(),
        get_drag_state=lambda: _floating_drag_state,
        set_drag_state=_set_floating_drag_state,
        is_topmost=(
            lambda: desktop_ui_core.floating_window_z_order(
                get_settings_snapshot()
            ) == "topmost"
        ),
        logger=logging,
