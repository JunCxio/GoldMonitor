...
- 支持行情质量持续异常通知，默认异常累计 5 分钟后通过本机提醒，同一轮异常只通知一次；可调整门槛和本机、邮件、Webhook 渠道。重启后续接应用运行期间已累计的异常时长，恢复时自动关闭原待处理项，并可发送恢复通知。警报弹窗、警报记录和今日概览可直接进入对应异常区间复盘。
...
- `market_quality_alert_state.json`：版本化行情质量异常通知状态，保存当前异常、运行期间累计时长、通知去重和最近恢复结果；应用关闭期间不累计异常时长。
