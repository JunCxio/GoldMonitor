...
- 支持按每天、每周、每月或每年设置人民币与美元固定金额定投计划，可选开始日期和结束日期，并按执行时最新有效行情生成本地买入流水；可预览未来 5 个执行日期，临时跳过当前期次而不暂停后续计划，可复制已有计划为默认暂停的新草稿，也可归档已结束或暂不使用的计划并保留绩效与执行记录。按计划可查看累计投入、执行次数、定投均价、当前市值、盈亏和最近执行记录，并将该计划的全部执行记录导出为 CSV。应用关闭期间只补最近一期，不连接交易平台或执行真实下单。
