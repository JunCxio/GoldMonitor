...
- 支持按每天、每周、每月或每年设置人民币与美元固定金额定投计划，可选开始日期、结束日期和目标期数，并按执行时最新有效行情生成本地买入流水；达到目标期数后自动完成。可预览未来 5 个执行日期、目标总预算、剩余投入和预计完成日期，并在定投概览中对照近 30 天实际投入、近 6 个月投入趋势、近 90 天执行稳定性与未来 30 天计划投入，查看每个计划的预计期数、单期金额和首末执行日期。支持按状态筛选和按执行时间、更新时间、累计投入或名称排序，临时跳过当前期次、复制计划、归档计划，并在单个计划详情中查看近 90 天按时、补执行和手动执行情况、计划买入金额与包含手续费的实际支出对照、累计绩效与最近执行记录，以及导出该计划的全部执行记录。投入对照反映本地流水中的手续费与数量舍入差额，不代表真实成交滑点；缺少计划金额的旧流水不会计入。应用关闭期间只补最近一期，不连接交易平台或执行真实下单。
...
- 行情质量使用可解释的连续评分，综合实时取数、缓存回退、当前主源滚动成功率、连续失败、数据新鲜度和跨源价差，页面会显示逐项扣分依据。
...
- `portfolio_transactions.json`：本地持仓流水。
...
运维设置中的“完整数据归档”会备份上述可恢复状态和 SQLite 数据库，但不包含更新安装包、普通导出产物或运行日志。完整归档包含密钥原文等敏感信息，应作为本机敏感备份保管；普通配置备份仍不包含密钥原文。
