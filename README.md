...
- 后台每 6 小时执行一次只读历史数据检查，发现数据库完整性、汇总差异或可补记录时会在运维任务中提示；连续 3 次异常后发送本机通知并进入今日概览的“运维”分类。数据修复仍需用户查看预览并明确确认，重建时会清理可还原范围内的多余汇总并保留范围外的长期数据；修复完成后会立即复检后台任务并刷新今日概览状态。
