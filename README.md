...
- 支持按每天、每周、每月或每年设置人民币与美元固定金额定投计划，可选开始日期、结束日期和目标期数，并按执行时最新有效行情生成本地买入流水；达到目标期数后自动完成。可预览未来 5 个执行日期、目标总预算、剩余投入和预计完成日期，并在定投概览中分别汇总未来 30 天的人民币与美元计划投入，查看每个计划的预计期数、单期金额和首末执行日期。支持临时跳过当前期次、复制计划、归档计划、查看绩效与最近执行记录，以及导出单个计划的全部执行记录。应用关闭期间只补最近一期，不连接交易平台或执行真实下单。
