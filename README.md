...
- 运维页提供行情可信度诊断视图，集中显示历史入库、预警判断和定投执行门禁，金价与汇率的来源时效、缓存状态、近期质量事件和数据源滚动健康指标。
- 行情质量事件会跨重启保留最近 30 天、最多 2000 段状态；运维页汇总最近 24 小时和 7 天的可信区间占比、异常时长，以及历史入库、预警和定投受到的影响。
- 支持行情质量持续异常通知，默认异常累计 5 分钟后通过本机提醒，同一轮异常只通知一次；可调整异常门槛、本机/邮件/Webhook 渠道，以及 1、2、3、5 或 10 分钟恢复确认门槛。行情需要默认连续正常 2 分钟才确认恢复，期间再次异常会取消确认但不会重复通知；重启后续接应用运行期间已累计的异常和恢复确认时长，停机时间不计入。确认恢复后自动关闭原待处理项，并可发送恢复通知。警报弹窗、警报记录和今日概览可直接进入对应异常区间复盘。
- 支持默认关闭的局域网只读面板，可通过独立端口和登录口令在可信局域网内查看行情、行情质量、规则状态与最近警报摘要。该端口不提供 Socket.IO、设置、归档、持仓、风险分析或警报处理等远程写入能力。
...
当前版本聚焦本地黄金行情监控、持仓管理、提醒通知和复盘分析，并已在明确的访问控制与只读数据契约下提供局域网只读面板。多品种贵金属、更多通知渠道和云同步属于后续扩展方向，启动前需先满足 `docs/product-discovery/extension-readiness-checklist.md` 中的模块边界、数据能力、核心闭环和行情可信度前置条件。
...
- `market_quality_history.json`：版本化行情质量历史，默认保留最近 30 天且最多 2000 段连续状态；应用关闭期间不计入异常持续时间。
- `market_quality_alert_state.json`：schema v2 行情质量异常通知状态，保存当前异常、运行期间累计时长、恢复确认进度、通知去重和最近恢复结果；兼容 v1 状态，应用关闭期间不累计异常或恢复确认时长。
...
SMTP 授权码、DeepSeek API Key、OpenAI 兼容接口 API Key 和局域网只读面板访问口令会优先保存到系统凭据存储：
