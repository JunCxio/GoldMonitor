...
def test_storage_manifest_covers_current_persistent_paths():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        paths = {
            ...
            "portfolio_investment_plans": str(tmp / "portfolio_investment_plans.json"),
    ...
    assert manifest["portfolio_investment_plans"]["schema"] == "item_payload"
    ...
    assert manifest["today_overview_state"]["schema"] == "versioned_object"
    assert manifest["today_overview_state"]["expected_schema_version"] == 1
...
def test_app_diagnostics_report_includes_complete_storage_manifest(monkeypatch, tmp_path):
    ...
    path_keys = {
        ...
        "PORTFOLIO_INVESTMENT_PLANS_PATH": tmp_path / "portfolio_investment_plans.json",
        ...
        "TODAY_OVERVIEW_STATE_PATH": tmp_path / "today_overview_state.json",
    ...
    assert report["storage_manifest"]["portfolio_investment_plans"]["schema"] == "item_payload"
    ...
    assert report["data_schemas"]["today_overview_state"]["expected_schema_version"] == 1
    assert report["data_schemas"]["source_metrics"]["expected_schema_version"] == 1
    assert report["data_schemas"]["market_quality_history"]["expected_schema_version"] == 1
