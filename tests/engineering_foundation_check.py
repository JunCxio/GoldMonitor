...
def test_diagnostics_summary_reports_retry_queue_attention():
    from goldmonitor.diagnostics import build_health_summary
    ...
    summary = build_health_summary(
        ...
        paths={},
        background_tasks={
            "summary": {"queue_attention": 1},
        },
    )

    assert summary["status"] == "degraded"
    assert summary["counts"]["background_task_queue_attention"] == 1
    assert "通知重试队列存在待处理项目且自动重试未开启" in summary["messages"]
